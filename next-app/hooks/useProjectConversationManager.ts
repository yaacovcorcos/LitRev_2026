...
import type { ConversationListItem } from "@/types/project-conversation-context";
import {
    listConversations,
    getConversation,
...
export type ProjectConversationManagerDeps = {
    projectId: string;
    routeConversationId?: string | null;
    updateState: (updater: (prev: ProjectConversationState) => ProjectConversationState) => void;
    setState: React.Dispatch<React.SetStateAction<ProjectConversationState>>;
...
export function useProjectConversationManager(deps: ProjectConversationManagerDeps) {
    const {
        projectId,
    ...
    const selectConversation = useCallback(async (conversationId: string): Promise<boolean> => {
        ...
        try {
            ...
            if (convo) {
                ...
                // Convert conversation messages to project conversation messages
                const persistedProjectConversationMessages: ProjectConversationMessage[] = convo.messages
                ...
                const oldestMessageTime = persistedProjectConversationMessages.length > 0
                    ? new Date(persistedProjectConversationMessages[0].createdAt).getTime()
                    : 0;
                const projectConversationArtifactMessages: ProjectConversationMessage[] = (convo.artifacts ?? [])
                ...
                const projectConversationMessages: ProjectConversationMessage[] = [...persistedProjectConversationMessages, ...projectConversationArtifactMessages].sort(
    ...
    const loadOlderMessages = useCallback(async () => {
        ...
        try {
            ...
            const olderProjectConversationMessages: ProjectConversationMessage[] = result.data.messages
                .filter((m) => m.role !== "system")
                .map((m) => ({
                    id: m.id,
            ...
            const oldestNewTime = olderProjectConversationMessages.length > 0
                ? new Date(olderProjectConversationMessages[0].createdAt).getTime()
                : Infinity;
            const existingMessageIds = new Set(currentMessages.map((m) => m.id));
            const allArtifacts = Array.from(artifacts.values());
            const newlyVisibleArtifactMessages: ProjectConversationMessage[] = allArtifacts
                .filter((a) => {
            ...
            const combined = [...olderProjectConversationMessages, ...newlyVisibleArtifactMessages].sort(
                (a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime(),
            );
