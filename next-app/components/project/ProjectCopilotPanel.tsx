...
import { ProjectConversationAutonomySettings } from "./ProjectConversationAutonomySettings";
import { decideProjectCopilotPanelWheelContainment } from "./project-copilot-panel-scroll-containment";
...
import styles from "./ProjectCopilotPanel.module.css";
...
export type ProjectCopilotPanelProps = {
...
export function ProjectCopilotPanel({
    ...
    const seededAutoAppliedStudyUpdateScopeRef = useRef<string | null>(null);
    ...
    const autoAppliedStudyUpdateScopeKey = `${projectId ?? "no-project"}:${currentConversationId ?? "no-conversation"}:${page}:${studyId ?? "no-study"}`;

    if (seededAutoAppliedStudyUpdateScopeRef.current !== autoAppliedStudyUpdateScopeKey) {
        const seededIds = new Set<string>();
        if (page === "study" && studyId && projectId) {
            for (const message of messages) {
                const artifact = message.artifact;
                if (!artifact || artifact.type !== "study_update" || artifact.status !== "auto_applied") continue;
                const payload = artifact.payload;
                if (!isStudyUpdatePayload(payload) || payload.studyId !== studyId) continue;
                seededIds.add(artifact.id);
            }
        }
        seenAutoAppliedStudyUpdatesRef.current = seededIds;
        seededAutoAppliedStudyUpdateScopeRef.current = autoAppliedStudyUpdateScopeKey;
    }
    ...
    const handleWheelCapture = useCallback((event: ReactWheelEvent<HTMLElement>) => {
        const decision = decideProjectCopilotPanelWheelContainment({
            target: event.target,
