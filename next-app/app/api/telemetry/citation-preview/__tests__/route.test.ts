...
const mocks = vi.hoisted(() => ({
    ...
    ingestCitationPreviewMetric: vi.fn(),
}));
...
vi.mock("@/lib/server/citation-preview-metrics", () => ({
    ingestCitationPreviewMetric: (...args: unknown[]) => mocks.ingestCitationPreviewMetric(...args),
}));

const { POST } = await import("@/app/api/telemetry/citation-preview/route");

function makeRequest(body: unknown): Request {
    return new Request("http://localhost/api/telemetry/citation-preview", {
        method: "POST",
...
describe("POST /api/telemetry/citation-preview", () => {
    const consoleErrorSpy = vi.spyOn(console, "error").mockImplementation(() => {});

    beforeEach(() => {
        vi.clearAllMocks();
        mocks.requireApiSession.mockReset();
        mocks.ingestCitationPreviewMetric.mockReset();
    ...
    it("returns 400 for invalid telemetry payloads", async () => {
        ...
        mocks.ingestCitationPreviewMetric.mockRejectedValue(new z.ZodError([]));
    ...
    it("returns opaque 500 errors and logs details server-side", async () => {
        ...
        mocks.ingestCitationPreviewMetric.mockRejectedValue(new Error("internal details: secret"));
