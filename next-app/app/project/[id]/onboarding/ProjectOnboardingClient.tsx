...
import { saveProtocolAction } from "@/app/actions/protocols";
import {
...
import { ONBOARDING_STEPS, type OnboardingStepId, type OnboardingStepStatus } from "@/lib/durable-route-state";
...
type ProjectOnboardingClientProps = {
  projectId: string;
  initialProtocol: ProtocolData;
  initialStepStatuses: Record<OnboardingStepId, OnboardingStepStatus>;
...
export function ProjectOnboardingClient({
  ...
  initialProtocol,
  initialStepStatuses,
  initialDerivedProfile,
}: ProjectOnboardingClientProps) {
  ...
  const [protocol, setProtocol] = useState<ProtocolData>(initialProtocol);
  const [topicText, setTopicText] = useState(initialProtocol.researchQuestion);
  ...
  const [derivedProfile, setDerivedProfile] = useState<OnboardingDerivedProfile | null>(initialDerivedProfile);
  ...
  const persistProgress = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const saveProtocolResult = await saveProtocolAction(projectId, hydratedProtocol);
      ...
      const onboardingResult = await saveProjectOnboardingProgressAction(projectId, {
        stepStatuses,
  ...
  }, [derivedProfile, hydratedProtocol, projectId, stepStatuses]);
  ...
  const persistAndContinue = useCallback(
    async (nextRoute: string) => {
      ...
      setIsSaving(true);
      try {
        const markResult = await markProjectOnboardingCompletedAction(projectId, { skipped: false });
    ...
    [persistProgress, projectId, router],
  );
        ...
        <div className={styles.headerActions}>
          ...
          <button
            ...
            onClick={async () => {
              ...
              const saved = await persistProgress();
              if (!saved) return;
              const skipResult = await markProjectOnboardingCompletedAction(projectId, { skipped: true });
            ...
            {stepIndex < ONBOARDING_STEPS.length - 1 ? (
              ...
              <div className={styles.finalActions}>
                <button
                  ...
                  className="btn btn-outline"
                  disabled={isSaving}
                  onClick={() => persistAndContinue(`/project/${projectId}`)}
                >
                ...
                <button
                  ...
                  onClick={() => persistAndContinue(`/project/${projectId}/protocol`)}
                >
