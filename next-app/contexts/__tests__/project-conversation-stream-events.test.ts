...
describe("project conversation stream event handlers", () => {
  ...
  it("creates and updates assistant message for content chunks", () => {
    ...
    const deps = {
      ...
      updateMessages: (updater: (messages: ProjectConversationMessage[]) => ProjectConversationMessage[]) => {
    ...
    let nextState = handleProjectConversationStreamChunk(
      { type: "content", content: "Hello" },
      baseState(),
      deps
    );
    nextState = handleProjectConversationStreamChunk(
  ...
  it("stores progress as structured state instead of assistant transcript text and clears it", () => {
    ...
    const deps = {
      ...
      updateMessages: (updater: (msgs: ProjectConversationMessage[]) => ProjectConversationMessage[]) => {
        const next = updater(messages);
        messages.splice(0, messages.length, ...next);
      },
      emitLedgerChanged: vi.fn(),
    ...
    let state = handleProjectConversationStreamChunk(
    ...
    state = handleProjectConversationStreamChunk(
  ...
  it("moves the reserved assistant behind live trace messages and reuses that row for the final answer", () => {
    ...
    let state = reserveProjectConversationAssistantTurn(baseState(), deps);
    state = handleProjectConversationStreamChunk(
  ...
  it("updates a pending ask_user card when a structured resolution is replayed", () => {
    ...
    const state = handleProjectConversationStreamChunk(
      {
