...
export type SearchProviderThrottleStore = {
  reserve(
    ...
    minIntervalMs: number,
    maxQueueWaitMs: number,
  ): Promise<{ reservedAt: Date; dbNow: Date }>;
  readCooldown(providerKey: string): Promise<{ cooldownUntil: Date | null; dbNow: Date }>;
...
export const prismaSearchProviderThrottleStore: SearchProviderThrottleStore = {
  async reserve(providerKey, minIntervalMs, maxQueueWaitMs) {
    const rows = await prisma.$queryRaw<Array<{ reservedAt: Date; dbNow: Date }>>`
      ...
      RETURNING
        "nextAvailableAt" - (${minIntervalMs} * interval '1 millisecond') AS "reservedAt",
        timezone('UTC', clock_timestamp()) AS "dbNow"
    `;
    ...
    return { reservedAt, dbNow };
  },

  async readCooldown(providerKey) {
    const rows = await prisma.$queryRaw<Array<{ cooldownUntil: Date | null; dbNow: Date }>>`
      SELECT
        "cooldownUntil",
        timezone('UTC', clock_timestamp()) AS "dbNow"
      FROM "SearchProviderThrottle"
      WHERE "providerKey" = ${providerKey}
    `;
    const row = rows[0];
    if (!row || !(row.dbNow instanceof Date) || !Number.isFinite(row.dbNow.getTime())) {
      throw new SearchProviderThrottleBusyError(providerKey, 1_000);
    }
    return row;
  },
  ...
  async defer(providerKey, delayMs) {
    await prisma.$executeRaw`
      INSERT INTO "SearchProviderThrottle" (
        "providerKey",
        "nextAvailableAt",
        "cooldownUntil",
        "updatedAt"
      ...
      ON CONFLICT ("providerKey") DO UPDATE SET
        ...
        "cooldownUntil" = GREATEST(
          "SearchProviderThrottle"."cooldownUntil",
          timezone('UTC', clock_timestamp()) + (${delayMs} * interval '1 millisecond')
        ),
        "updatedAt" = timezone('UTC', clock_timestamp())
    `;
...
export class SearchProviderThrottle {
  ...
  async wait(provider: SearchProvider, signal?: AbortSignal): Promise<void> {
    ...
    let reservation = await this.store.reserve(
      providerKey,
    ...
    while (true) {
      throwIfAborted(signal);
      const reservationDelayMs = Math.max(
        0,
        reservation.reservedAt.getTime() - reservation.dbNow.getTime(),
      );
      if (reservationDelayMs > 0) {
        await (this.runtime.sleep ?? sleep)(reservationDelayMs, signal);
      }

      throwIfAborted(signal);
      const cooldown = await this.store.readCooldown(providerKey);
      if (
        !cooldown.cooldownUntil
        || cooldown.cooldownUntil.getTime() <= reservation.reservedAt.getTime()
      ) {
        return;
      }

      // A Retry-After persisted after this slot was reserved invalidates the
      // stale slot. Reserve again so previously sleeping and newly arriving
      // callers remain spaced when the shared cooldown expires.
