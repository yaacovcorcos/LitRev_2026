...
describe("getToolDefinitions", () => {
    ...
    it("returns only global-safe tools in global scope", () => {
        ...
        expect(names).toEqual(["ask_user", "create_project", "delegate_protocol", "delegate_screening", "delegate_search", "forget_memory", "inspect_memory", "list_projects", "open_project", "search_openalex", "search_pubmed", "search_semantic_scholar", "store_memory"]);
    });
    ...
    it("applies scope filter before mode filter", () => {
        ...
        const names = defs.map((d) => d.name).sort();
        expect(names).toEqual(["ask_user", "forget_memory", "inspect_memory", "search_openalex", "search_pubmed", "search_semantic_scholar", "store_memory"]);
    });
