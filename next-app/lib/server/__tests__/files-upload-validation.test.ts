...
describe("uploadStudyFile server validation", () => {
  ...
  it("fails closed when any requested file is missing or outside the authorized project", async () => {
    mocks.prisma.fileAsset.findMany.mockResolvedValue([]);
    ...
    )).rejects.toThrow("unavailable or outside this project");

    expect(fetch).not.toHaveBeenCalled();
  });

  it("validates found PDF references while hydrating only found images", async () => {
    mocks.prisma.fileAsset.findMany.mockResolvedValue([
      createdFileAsset(),
      {
        ...createdFileAsset({
    ...
    expect(images).toEqual([
      expect.objectContaining({ fileAssetId: "image-1", mimeType: "image/png" }),
    ]);
    expect(fetch).toHaveBeenCalledTimes(1);
  });
