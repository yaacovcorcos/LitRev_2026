...
- The protected CI `check` gate now runs `npm run check:agent-quality`, which first executes the deterministic runtime-scenario matrix and then guards the eval catalog, runtime-signal fixture coverage, and strict burn-in metric/threshold contract before a runtime PR can merge.
