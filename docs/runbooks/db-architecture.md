...
| AI chat and telemetry | `AIConversation`, `AIMessage`, `AIUsageReservation`, `AIUsage`, `ChatUnificationMetric` | Conversation storage, message timeline, durable provider-attempt quota admission, token/cost attribution, chat surface telemetry |
...
| `AIUsage` | Settled token/cost attribution records | `conversationId -> AIConversation`, `projectId -> Project`, optional `reservationId -> AIUsageReservation` | `reservationId` unique; indexes on user/workspace/project/conversation/source/contextPage + `createdAt` | `reservationId` is null for legacy/non-provider accounting; user/workspace/project/conversation are optional because attribution can vary by surface/context | No |
...
| `AgentRun` | Top-level agent execution trace | `projectId -> Project`, `parentRunId -> AgentRun` | Indexes on project/conversation/user/lineage lifecycle fields plus `memoryExtractionStatus + memoryExtractionLeaseExpiresAt` | Scope, lineage, model, completion, memory-extraction lease/completion/error fields are intentionally nullable; lifecycle fields record phase, liveness, durable progress, finalization, durability, abnormal exits, and the durable `pending | processing | succeeded | skipped | failed` conversation-memory extraction state | No |
