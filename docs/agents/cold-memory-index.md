...
| Security baseline, disclosure policy, and cross-cutting security review | `SECURITY.md`, `docs/runbooks/security-baseline.md`, plus owner docs for the touched subsystem (`docs/plans/plan-backend.md`, `docs/runbooks/admin-access.md`, `docs/plans/plan-agentic.md`, `docs/plans/plan-memory.md`, `docs/plans/plan-agent-quality.md`, `docs/runbooks/db-architecture.md`) | Any security review, vulnerability triage, disclosure-policy update, or request to establish security standards |
| Agent orchestration and execution loop | `docs/plans/plan-agentic.md`, `docs/plans/plan-agent-quality.md`; use `docs/plans/README.md` to confirm active ownership; use `docs/design/agent-decision-system.md` only when working on the next `ask_user` decision-system wave; `next-app/lib/agent/**`, `next-app/lib/server/agent/**` | Changes to planner/execution/router/sub-agent flow; use only active plans listed in `docs/plans/README.md` |
...
| Prompting/extraction logic | `docs/plans/plan-memory.md` | Prompt text/schema/LLM extraction changes |
...
| Responsive/mobile foundation and chat UX execution | `docs/plans/mobile-plan.md`, `docs/plans/mobile-layout-contract.md`, `docs/runbooks/responsive-foundation-certification.md`, `docs/plans/plan-agentic.md` | Any responsive tier, phone/compact layout contract, mobile chat layout, drawer/composer, popup behavior, or responsive certification change |
| Chat runtime rollout operations | `docs/plans/plan-agent-quality.md`, `docs/runbooks/chat-runtime-burn-in.md`, `docs/plans/plan-agentic.md` | U1.6 canary setup, burn-in validation, and post-sign-off runtime cleanup decisions |
| Cross-cutting testing strategy, test-scope decisions, and testing execution improvements | `docs/agents/testing-agent-contract.md`, `docs/runbooks/testing-ci-strategy.md`, `docs/plans/plan-testing-execution.md`, `docs/plans/plan-lint-governance.md`, `docs/reviews/repo-health.md` | Any request to decide what tests to add, evaluate regression-proof coverage, define test strategy, improve test command or CI execution, or review whether a change is well tested |
| Browser tooling readiness and agent-driven browser verification | `docs/runbooks/browser-tooling-readiness.md`, `docs/agents/testing-agent-contract.md`, `docs/runbooks/responsive-foundation-certification.md` when responsive/mobile certification is involved | Any request to prepare a machine for Playwright or `agent-browser`, explain the local browser-testing stack, or troubleshoot local browser-verification readiness |
| Git branch/PR/review automation | `docs/runbooks/git-flow.md` | Any edits to `.github/workflows/**`, branch protections, CODEOWNERS, or agent git policy |
