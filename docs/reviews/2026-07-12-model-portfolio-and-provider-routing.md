...
|---|---|---:|---|---|---:|---:|---:|---:|---:|
| DeepSeek V4 Flash | Fast & cheapest | 1M / 384K | Extremely cheap text, coding chores, extraction, tools | Fast, High, Max | $0.140 | $0.0028 | — | $0.280 | $0.084 |
...
- Grok 4.5 direct xAI pricing is flat through its published 500K context: $2.00 input, $0.50 cached input, and $6.00 output per million.
...
- Agent runs and AI usage persist requested and actual model/provider/effort/delivery metadata. Usage rows separately persist cache-hit input, provider-reported cache-write input, reasoning tokens, and the resulting tier-aware estimated USD cost.
- Invisible/background work prefers Flash or Pro only after the corresponding gateway route is authenticated and rollout-enabled; it safely falls back to Luna before activation, and bounded JSON jobs use Fast reasoning.
