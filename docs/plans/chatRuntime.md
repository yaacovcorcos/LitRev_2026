...
- The main surfaces now derive their UI-facing live state from shared runtime facts instead of shell-local phase toggles alone: pending user input, recovery recommendations, terminal run facts, and running-tool state are carried through the shared reducer/runtime path and used to keep queued follow-up dispatch locked until the run is truly sendable again.
