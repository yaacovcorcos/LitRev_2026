...
- **Search Result Year Contract:** Search adapters may now return `SearchResult.year` as unknown when the source metadata is missing or malformed; PubMed no longer fabricates the current year. Ledger persistence remains stricter than search recall: `add_to_ledger` skips yearless results with explicit reporting, while title+year dedupe only applies when a trustworthy year exists and identifier-based dedupe continues to work without one.
...
- [ ] Normalize **Cross-Provider Search Year Honesty**:
  - Remove current-year fabrication from OpenAlex and Semantic Scholar so missing publication years remain explicit instead of being guessed.
  - Reconcile downstream year filtering and ranking behavior for unknown-year results.
  - Decide whether the ledger/import contract should remain skip-on-missing-year or evolve toward explicit unresolved-year review flows.
## Recently Completed
