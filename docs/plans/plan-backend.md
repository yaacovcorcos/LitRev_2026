...
- **File Asset Namespace Contract:** `FileAsset` writes now accept only canonical project-scoped storage object paths under `projects/<projectId>/...` (optionally prefixed by the `study-assets` bucket). Backend validation normalizes bucket/object path segments before comparison so project tenancy is enforced by namespace, not substring matching.
...
- [x] Hardened file-asset project scoping in `lib/server/files.ts`: backend writes now normalize storage path segments, accept only canonical project namespaces, reject project-ID substring lookalikes, and cover the boundary with focused service tests.
