...
- Guided setup is opt-in by default: Home and first-login paths must never auto-route to onboarding or a first-run welcome gate. Authenticated Home renders the workspace shell even when there are no projects; setup access stays behind explicit create actions. User-level `UserMemory` (`guided_setup_new_projects`) can explicitly re-enable launch behavior, and project-level `enabledOverride` can still override the user default.
...
- **No first-login/Home auto-launch or welcome gate.** Guided setup is a contextual opt-in helper, not a forced Home or dashboard gate.
...
## Recently Completed
- [x] Home and first-login onboarding launch behavior is now opt-in by default: the backend default is disabled, authenticated Home opens directly to the workspace shell for empty accounts, and guided setup access is explicit when the availability gate is reopened.
