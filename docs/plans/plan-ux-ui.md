...
- Agent platform/runtime truth: `docs/plans/plan-agentic.md`
- Agent quality and rollout gates: `docs/plans/plan-agent-quality.md`
...
- Streaming action safety and suggestion prefill hardening are active:
  - `useStreamingGate()` is exported from `ProjectConversationContext`.
...
- Shared scoped-context UI is active:
  - `ProjectCopilotPanel` / `ChatComposerCore` now render typed context receipts and recent-context history chips owned by `docs/plans/plan-context-capture.md`.
  - `PopupChat` shows compact context previews, keeps edit intents advisory-only via `Continue in Copilot`, now promotes popup context into full copilot through structured `context_capture` attachment handoff instead of transcript-only copying, and draft desktop quick actions are gated behind the dedicated context-toolbar flag rather than leaking into mobile flows.
- Main timeline surfaces now support conservative completed-turn execution-trace compaction:
  ...
  - artifact cards on the main timeline surfaces now use one shared inline action policy: `ChatTimeline` orchestrates confirmations and one mutation lane, `ArtifactWrapper` owns shared settled affordances, destructive review resolutions and undo use selective confirmation, inline undo is initially allowlisted for `study_update` only, and manual review stays visibly pending until the server confirms success instead of optimistically rendering `accepted`.
  - grouping is renderer-only, skips ambiguous/blocking/error cases, and leaves popup out of scope.
  - shared `tool_activity` cards on the main timeline surfaces now prefer additive semantic receipt fields (`displayLabel`, `inputPreview`, `outcomeSummary`, `sourceBadge`, `detailItems`) derived once in the shared reducer/runtime path for search, read/inspection, and delegation tools, while uncovered tool families stay on truthful fallback rendering and popup remains an intentionally reduced trace subset that now exposes only compact settled semantic receipts from that same shared path.
- Shared composer action hierarchy is active across chat surfaces:
  - `ChatComposerCore` now uses a left-edge `+` extension menu for secondary actions, keeps voice in the right-side primary action cluster beside send, and presents a real microphone-driven waveform/timer state while recording.
  ...
  - Shared model selection is now a controlled surface-owned contract: `ChatComposerCore` no longer reads or writes localStorage itself, while `/ai` and project copilot keep any durable model preference at their owning shell/context layer instead of inside the shared composer primitive.
  - Shared model configuration now uses one seven-model selector across desktop and mobile, with concise role/cost cues, provider-readiness disabled states, a premium Sol confirmation, and a narrow-width trigger that remains reachable instead of disappearing.
  - Reasoning effort is a capability-driven model-specific selector. Reasoning visibility is hidden for the current portfolio because direct OpenAI/xAI do not return a supported summary and DeepSeek/Qwen expose raw private reasoning rather than a safe summary. Paid Faster delivery is a separate one-shot control shown only for supported routes.
  - `/ai` mobile exposes model, effort, and conditional delivery in an accessible dialog, while project main conversation, side copilot, plans, retries, and queued follow-ups consume the same owner-held preference snapshot; popup is explicitly standard-delivery only.
  - Conversation attachments accept signature-validated PDF, PNG, JPEG, and WebP inputs; visually capable models receive bounded, deduplicated, access-checked current-turn image parts rather than client-authored raw payloads. Historical image replay remains a separate attachment-aware history-hydration task.
  ...
  - Save-to-notes flows now use truthful user feedback instead of silent failure: main timeline assistant rows show inline failure + retry state on the affected message, popup footer save shows calm saved/error feedback with retry, and shared callers now treat failed note-creation results as real errors instead of ignoring unsuccessful action payloads.
...
- [ ] `CUX-039` Make chat surfaces refresh-safe with URL-addressable conversation identity.
  ...
  - Guardrails:
    - preserve the shared runtime contract owned by `plan-agentic.md`
...
- [x] Inline artifact action contract shipped on the main timeline surfaces: one shared artifact action policy now drives inline review controls, destructive review resolutions and `Undo` use selective confirmation through the shared confirm dialog, `ChatTimeline` enforces a single artifact mutation lane, `ArtifactWrapper` owns shared settled affordances, and inline undo is initially allowlisted for `study_update` while popup remains unchanged.
...
- [x] Progressive answer streaming is now enabled by default on the shared `/ai`, main conversation, and side-panel runtime contract: send reserves the assistant turn immediately, the same row is later populated by streamed answer content, progress clears once through the shared reducer handoff, live reasoning stays collapsed/non-dominant by default, popup now uses the shared runtime path for its reduced surface while remaining intentionally compact, and env flags now act as an emergency opt-out instead of the primary rollout switch.
...
- [x] Artifact review visibility contract shipped: `proposed` artifacts now stay inline until resolved instead of disappearing into `Process details`, manual review uses a pending mutation lane and never renders optimistic `accepted`, batch approval still derives only from valid persisted proposed artifacts, and settled artifact cards standardize on explicit read-only approved/rejected/applied states instead of leaving stale approval controls visible.
