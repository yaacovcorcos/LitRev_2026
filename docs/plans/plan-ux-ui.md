...
- Main timeline surfaces now support conservative completed-turn execution-trace compaction:
  ...
  - artifact cards on the main timeline surfaces now use one shared inline action policy: `TimelineRenderer` orchestrates confirmations and one mutation lane, `ArtifactWrapper` owns shared settled affordances, destructive review resolutions and undo use selective confirmation, inline undo is initially allowlisted for `study_update` only, and manual review stays visibly pending until the server confirms success instead of optimistically rendering `accepted`.
...
- Shared composer action hierarchy is active across chat surfaces:
  ...
  - Shared model selection is now a controlled surface-owned contract: `CopilotInputCore` no longer reads or writes localStorage itself, while `/ai` and project copilot keep any durable model preference at their owning shell/context layer instead of inside the shared composer primitive.
