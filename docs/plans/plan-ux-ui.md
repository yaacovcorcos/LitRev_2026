...
## Recently Completed
- [x] Progressive answer streaming shipped on the shared `/ai`, main conversation, and side-panel runtime contract behind a dedicated feature flag: send now reserves the assistant turn immediately, the same row is later populated by streamed answer content, progress clears once through the shared reducer handoff, live reasoning stays collapsed/non-dominant by default, and popup remains on the older path until a separate rollout is approved.
