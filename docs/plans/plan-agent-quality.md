...
- Selectable provider compatibility now has an opt-in live portfolio smoke that sends tools with each model's default reasoning, can require all public routes to be configured, and can require provider-confirmed priority receipts for paid delivery modes. Deterministic adapter tests remain the primary regression layer; the live smoke is release/incident evidence for provider contracts mocks cannot prove.
