...
- [x] Implemented `FIX-012a` run finalization guard: started runs are now created inside the guarded `streamChatWithArtifacts` lifecycle, `run_start` is only emitted after that guard is active, and a top-level `finally` force-finalizes any started run that would otherwise leak as `running` after early stream termination.
