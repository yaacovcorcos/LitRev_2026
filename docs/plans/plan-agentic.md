...
- **Checkpointed Retry Now Generalizes Durable Continuation Across Retry/Replace Flows:** the stream entrypoint now resolves continuation through one server-owned `checkpoint -> durable -> fresh retry` selector, explicit `Continue` remains a strict proven-state path, and run-targeted retry/replace actions on the main surfaces can now reuse audited durable work when it exists without breaking clean fallback to a fresh retry when it does not.
...
- **Popup Now Uses The Shared Runtime Contract For Its Reduced Surface:** popup now derives its supported progress, checkpoint, settled semantic receipt, blocking-clarification, and structured terminal-error states through the shared runtime controller/reducer path instead of bespoke chunk-only rendering, keeps popup-only capability limits in explicit surface config rather than a separate runtime model, and promotes into full copilot through structured `context_capture` handoff instead of transcript-only copying.
...
2. remaining roadmap phases after the runtime closeout contract is stable
...
- [x] `CAG-003` Generalized the shipped durable continuation/recovery foundation into a broader checkpointed retry/continue capability so reconnect / continue / replace / retry semantics stop restarting from zero when audited durable work already exists
...
- [x] Shipped `CAG-003` checkpointed retry generalization: explicit `Continue` still requires a proven checkpoint or durable source, but run-targeted retry/replace flows on `/ai`, the main project conversation, and side-panel copilot now request the same audited server-owned continuation path as a best-effort seed and fall back cleanly to a fresh retry when no safe source remains.
