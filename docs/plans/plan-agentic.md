...
- Tool boundaries are materially better:
  ...
  - PubMed, OpenAlex, and Semantic Scholar now reserve provider/hashed-credential pacing slots through one durable Postgres cursor instead of process-local timers; HTTP status and retry headers stay typed for the central executor, and 429 cooldowns are shared across instances
