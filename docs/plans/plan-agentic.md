...
- **Main-Surface Sendability Now Uses Shared Runtime Selectors:** `/ai`, main conversation, and side-panel copilot now derive UI-facing sendability from shared runtime state, so pending user input and recovery-required runs stay locked behind one explicit next action instead of being treated as idle by shell-local loading flags alone.
